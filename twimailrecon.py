...
async def send_email(username, proxy, useragent):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            authenticity_token = ""
            regex_output = re.search(r'authenticity_token.+value="(\w+)">', await res_begin.text() )
                ...
                async with session.post(
                    ...
                    "&account_identifier=" + username) as res_send:
                        regex_output = re.search(
                                r'<strong .+>([a-zA-Z]+\*+@[a-zA-Z\*\.]+)<\/strong>', await res_send.text() )
                        if regex_output and regex_output.group(1):
                            masked_email = regex_output.group(1)
                            print("[*] Succeed to get masked email.")
                            return_dict = {
                            "status": 0, 
                            "username": username,
                            ...
                            print("[!] Failed to get masked email.")
                        return_dict = {
                            "status": -1, 
                            "username": username,
                            "masked_email": ""
...
async def main():
    ...
    userlist_raw = input("[?] Please enter username by : ")

    userlist = userlist_raw.split(",")
    proxy = ""
    ...
    for user in userlist:
        tasks_list.append(asyncio.create_task( send_email(user, proxy, user_agents[random.randint(0,len(user_agents) - 1)]) ))

    for f in tqdm(asyncio.as_completed(tasks_list)):
        result = await f
        if result["status"] == 0:
            print("[*] {} 's email is {}. ".format(result["username"], result["masked_email"]))
            result_list.appned(result)
        else:
            print("[!] {} 's email is failed. ".format(result["username"]))

        with open("{}.pkl".format("hogehoge"), 'wb') as file:
